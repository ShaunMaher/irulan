// Enable various verbose logging of functions
//console.verbose.filterObjects.push(/\.page\./);
//console.verbose.filterObjects.push(/\.pages\./);
//console.verbose.filterObjects.push(/\.warning\./);
//console.verbose.filterObjects.push(/\.pageRenderer/);

// This will enable all verbose messages generated by this file
console.verbose.filterObjects.push("components.pages.js");

irulanclient.directive('pages', ['Pages', function directive$pages(Pages) {
  console.verbose("pages directive")
  //console.verbose($element)
  return {
    restrict: 'E',
    transclude: true,
    link: function directive$pages$link($scope, $element, $attrs, controller, $transclude) {
      $scope.pages = Pages.pages;
      //console.verbose($element);
      //console.verbose(Pages);
...
irulanclient.directive('page', ['$filter', '$timeout', '$compile', function directive$page($filter, $timeout, $compile) {
  return {
    restrict: 'E',
    link: function directive$page$link($scope, $element, $attrs, controller, $transclude) {
      console.verbose('inject page!');
      if (!$transclude) {
      ...
      //console.verbose($filter('pageRenderer')(page));
      //console.verbose($element)
      ...
      $scope.$watch(
        (function directive$page$watch$monitor() {
          return JSON.stringify($scope.pages[context.pagename]);
        }).bind(context),
        (function directive$page$watch$callback() {
          //window.setTimeout(
          $timeout((function directive$page$watch$callback$timeout() {
            console.verbose("page update detected");
...
irulanclient.directive('warning', function directive$warning() {
  console.verbose("warning directive")
  return {
    restrict: 'E',
    transclude: true,
    template:
...
irulanclient.factory('Page', function() {
  ...
  return {
    getNew: PageObject,
    get: function() {
      console.verbose("generic 'get' function");
    }
  }
})
...
irulanclient.service('Pages', ['Page', 'WorkspaceProviders', '$resource', function service$pages(Page, WorkspaceProviders, $resource){
  //this.Page = Page
  this.pages = {};
  this.element = 'test';

  console.verbose("Pages: WorkspaceProviders: " + JSON.stringify(WorkspaceProviders));

  this.get = function service$pages$get(options, callback) {
    for (index in WorkspaceProviders) {
      ...
      if (WorkspaceProvider.namespace) {
        if (options.name.substring(0, WorkspaceProvider.namespace.length) == WorkspaceProvider.namespace) {
          ...
          var newPage = WorkspaceProvider.factory.getNew();
          console.verbose("newPage");
          console.verbose(newPage);
  ...
  this.show = function service$pages$show(options, $scope, callback) {
    console.verbose("Show Pages scope:")
    console.verbose(this.pages);
    ...
    $scope.pages[options.name] = this.get(options, function service$pages$show$callback() {
      console.verbose("Show pages callback");
      callback();
    });
  }

  this.hide = function() {
    console.verbose("hide pages");
    console.verbose(this);

    // This gets rid of the Pages object's contents.  No idea how to put stuff
    //  back in again later though.  I also don't quite get what "this" is.
    //  More accurately, I know it's an object from the "WorkspaceProviders"
    //  config component, I just don't know why.  I was expecting it to be the
    //  "Pages" service.  So much more to learn.
    this.workspace.element.empty();
  }
...
irulanclient.filter('pageRenderer', ['$filter', 'SourceRenderers', 'Pages', function ($filter, SourceRenderers, Pages) {
  return function filter$pageRenderer(page) {
    ...
    if (page.sourceFormat) {
      ...
      console.verbose("Rendering page raw since it has no sourceFormat")
      return page
    }
  }
}])
